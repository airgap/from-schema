...
type Token = {
  type: 'string' | 'number' | 'bigint' | 'true' | 'false' | 'null' | 'undefined' |
         'object_start' | 'object_end' | 'array_start' | 'array_end' | 'colon' | 'comma';
  value: string;
  position: number;
};
...
function parse(tokens: Token[], startIdx = 0, depth = 0): { value: unknown; nextIdx: number } {
  // Guard against excessive recursion
  if (depth > MAX_NESTING_DEPTH) {
    throw new ParseError('Maximum nesting depth exceeded', tokens[startIdx]?.position || -1);
  }
  ...
  switch (token.type) {
    case 'string':
      ...
      if (token.value.length > MAX_STRING_LENGTH) {
        throw new ParseError(`String length exceeds maximum allowed (${MAX_STRING_LENGTH})`, token.position);
      }
    ...
    case 'number': {
      ...
      if (!Number.isFinite(num) && !Number.isNaN(num)) {
        throw new ParseError(`Invalid number value: ${token.value}`, token.position);
      }
    ...
    case 'bigint': {
      try {
        ...
        throw new ParseError(`Invalid BigInt value: ${token.value}`, token.position);
      }
    }
    ...
    case 'object_start': {
      ...
      while (nextIdx < tokens.length) {
        ...
        if (propertyCount > 100000) {
          throw new ParseError('Object contains too many properties', token.position);
        }
        ...
        if (key === '__proto__' || key === 'constructor' || key === 'prototype') {
          throw new ParseError(`Potentially unsafe property name: ${key}`, tokens[nextIdx].position);
        }
    ...
    case 'array_start': {
      ...
      while (nextIdx < tokens.length) {
        ...
        if (result.length > 10000000) { // 10 million elements
          throw new ParseError('Array contains too many elements', token.position);
        }
...
export const parseBON = (input: string): unknown => {
  try {
    ...
    if (input.length > MAX_INPUT_LENGTH) {
      throw new Error(`Input exceeds maximum allowed length (${MAX_INPUT_LENGTH})`);
    }
    ...
    if (nextIdx < tokens.length) {
      throw new ParseError(`Unexpected token after end of BON: ${tokens[nextIdx].type}`, tokens[nextIdx].position);
    }
