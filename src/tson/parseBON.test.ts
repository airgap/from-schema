...
describe('parseBON', () => {
	...
	it('parses objects correctly', () => {
		...
		expect(parseBON('{"a": "hello", "b": true, "c": null}')).toEqual({
			a: 'hello',
			b: true,
			c: null,
		});
	...
	it('round-trips complex objects correctly', () => {
		...
		const parsedClone = JSON.parse(
			JSON.stringify({
				...parsedObj,
				date: null,
				bigint: null,
				array: parsedObj.array.map((x: any) =>
					typeof x === 'bigint' ? null : x,
				),
				nested: {
					...parsedObj.nested,
					d: null,
...
describe('parseBON and stringifyBON interoperability', () => {
	...
	it('correctly handles special characters in strings', () => {
		...
		expect(typeof parseBON(stringifyBON('contains \\ backslash'))).toBe(
			'string',
		);
	});
});
