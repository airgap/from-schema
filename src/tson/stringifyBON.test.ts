...
describe('stringifyBON', () => {
	...
	it('handles primitive types correctly', () => {
		...
		expect(stringifyBON(BigInt(Number.MAX_SAFE_INTEGER) + 1n)).toBe(
			'9007199254740992n',
		);
	});
	...
	it('handles arrays correctly', () => {
		...
		expect(stringifyBON([true, false, null, undefined])).toBe(
			'[true, false, null, undefined]',
		);
		expect(stringifyBON([1, 'a', true, null, 42n])).toBe(
			'[1, "a", true, null, 42n]',
		);
	...
	it('handles objects correctly', () => {
		...
		expect(stringifyBON({ a: 'hello', b: true, c: null })).toBe(
			'{"a": "hello", "b": true, "c": null}',
		);

		// Nested objects
		expect(stringifyBON({ a: { b: { c: 1 } } })).toBe('{"a": {"b": {"c": 1}}}');
		expect(stringifyBON({ a: {}, b: { c: {} } })).toBe(
			'{"a": {}, "b": {"c": {}}}',
		);
	});
	...
	it('handles mixed nested structures', () => {
		...
		const expected =
			'{"array": [1, 2, 3], "object": {"a": 1, "b": 2}, "nestedArray": [{"a": 1}, {"b": 2}], "nestedObject": {"array": [1, 2, 3]}}';
	...
	it('escapes string content correctly', () => {
		...
		expect(stringifyBON('backslash \\ character')).toBe(
			'"backslash \\\\ character"',
		);
	...
	it('handles boundary values', () => {
		...
		expect(stringifyBON(Number.MAX_SAFE_INTEGER)).toBe(
			Number.MAX_SAFE_INTEGER.toString(),
		);
		expect(stringifyBON(Number.MIN_SAFE_INTEGER)).toBe(
		...
		expect(stringifyBON(new Date('9999-12-31T23:59:59.999Z'))).toBe(
